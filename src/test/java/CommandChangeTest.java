...
public class CommandChangeTest {
  ...
  public void validateTest() {
    ...
    Assert.assertEquals(CommandStatus.OK, test.validate("Change ff id 1000 price -10"));
    Assert.assertEquals(CommandStatus.OK, test.validate("change ff id 1000 price -10"));
    Assert.assertEquals(CommandStatus.OK, test.validate("Change ff id 1000 price +10"));
    Assert.assertEquals(CommandStatus.OK, test.validate("change ff id 1000 price +10"));
    Assert.assertEquals(CommandStatus.OK, test.validate("Change ff id 1000 price 0"));
    Assert.assertEquals(CommandStatus.OK, test.validate("change ff id 1000 price 0"));

    Assert.assertEquals(CommandStatus.BAD_LENGTH, test.validate("Change ff id 1000 stock -1000 stock"));
    ...
    Assert.assertEquals(CommandStatus.BAD_TYPE, test.validate("Change ffff id 1000 stock -1000"));
    Assert.assertEquals(CommandStatus.BAD_TYPE, test.validate("Change apple id 1000 stock -1000"));

    Assert.assertEquals(CommandStatus.BAD_ATTRIBUTE, test.validate("Change ff id 1000 name david"));
    Assert.assertEquals(CommandStatus.BAD_ATTRIBUTE, test.validate("Change ff stock 1000 stock -10"));


    Assert.assertEquals(CommandStatus.BAD_VALUE, test.validate("Change ff id 1000 price -1O"));
    Assert.assertEquals(CommandStatus.BAD_VALUE, test.validate("Change ff id 1000 price -1O"));
  ...
  public void parseTest() {
    ...
    Assert.assertEquals(CommandStatus.OK, test.parse("Change ff id 1000 price -10"));
    Assert.assertEquals(CommandStatus.OK, test.parse("change ff id 1000 price -10"));
    Assert.assertEquals(CommandStatus.OK, test.parse("Change ff id 1000 price +10"));
    Assert.assertEquals(CommandStatus.OK, test.parse("change ff id 1000 price +10"));
    Assert.assertEquals(CommandStatus.OK, test.parse("Change ff id 1000 price 0"));
    Assert.assertEquals(CommandStatus.OK, test.parse("change ff id 1000 price 0"));

    Assert.assertEquals(CommandStatus.BAD_LENGTH, test.parse("Change ff id 1000 stock -1000 stock"));
    ...
    Assert.assertEquals(CommandStatus.BAD_TYPE, test.parse("Change ffff id 1000 stock -1000"));
    Assert.assertEquals(CommandStatus.BAD_TYPE, test.parse("Change apple id 1000 stock -1000"));

    Assert.assertEquals(CommandStatus.BAD_ATTRIBUTE, test.parse("Change ff id 1000 name david"));
    Assert.assertEquals(CommandStatus.BAD_ATTRIBUTE, test.parse("Change ff stock 1000 stock -10"));


    Assert.assertEquals(CommandStatus.BAD_VALUE, test.parse("Change ff id 1000 price -1O"));
    Assert.assertEquals(CommandStatus.BAD_VALUE, test.parse("Change ff id 1000 price -1O"));
  ...
  public void runTest() {
    ...
    Assert.assertEquals(CommandStatus.OK, test.run("Change ff id 1000 price -10", storage));
    ...
    Assert.assertEquals(CommandStatus.OK, test.run("Change ff id 1000 price 0", storage));
    Assert.assertEquals(18.75, storage.find("name", "apples").getPrice(), 0.01);

    Assert.assertEquals(CommandStatus.BAD_LENGTH, test.run("Change ff id 1000 stock -1000 stock", storage));
    ...
    Assert.assertEquals(CommandStatus.BAD_TYPE, test.run("Change ffff id 1000 stock -1000", storage));
    Assert.assertEquals(CommandStatus.BAD_TYPE, test.run("Change apple id 1000 stock -1000", storage));
    Assert.assertEquals(CommandStatus.OK, test.run(storage));
    Assert.assertEquals(18.75, storage.find("name", "apples").getPrice(), 0.01);

    Assert.assertEquals(CommandStatus.BAD_ATTRIBUTE, test.run("Change ff id 1000 name david", storage));
    Assert.assertEquals(CommandStatus.BAD_ATTRIBUTE, test.run("Change ff stock 1000 stock -10", storage));
    Assert.assertEquals(CommandStatus.OK, test.run(storage));
    Assert.assertEquals(18.75, storage.find("name", "apples").getPrice(), 0.01);


    Assert.assertEquals(CommandStatus.BAD_VALUE, test.run("Change ff id 1000 price -1O", storage));
    Assert.assertEquals(CommandStatus.BAD_VALUE, test.run("Change ff id 1000 price -1O", storage));
    Assert.assertEquals(CommandStatus.OK, test.run(storage));
    Assert.assertEquals(18.75, storage.find("name", "apples").getPrice(), 0.01);
